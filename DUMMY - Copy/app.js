...
function formatGestureCandidates(gestureCandidates) {
return gestureCandidates
.filter((gesture) => gesture.categoryName !== "None" && gesture.score >= 0.35)
.slice(0, 3)
.map((gesture) => `${formatGestureName(gesture.categoryName)} (${Math.round(gesture.score * 100)}%)`)
.join(", ");
}
...
const summaries = results.gestures
.map((gestureCandidates, index) => {
const handName = results.handedness?.[index]?.[0]?.displayName || `Hand ${index + 1}`;
const summary = formatGestureCandidates(gestureCandidates);
return summary ? `${handName}: ${summary}` : null;
})
.filter(Boolean);

if (summaries.length > 0) {
setStatus(summaries.join(" • "));
} else {
